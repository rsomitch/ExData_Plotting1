...
#Plot 4
par(mfrow = c(2,2), mar = c(5,5,2,2), oma = c(2,2,4,2))
with(power2days, {
  plot(power2days$Time, as.character(power2days$Global_active_power), type = "l",
       xlab = "", ylab = "Global Active Power (kilowatts)")
  
  plot(power2days$Time, as.character(power2days$Voltage), type = "l", xlab = "datetime", ylab = "Voltage")
  
  plot(power2days$Time, as.character(power2days$Sub_metering_1),type = "l", xlab = "", ylab = "Energy Sub metering") 
  with(power2days, lines(Time, as.character(power2days$Sub_metering_2), col = "red"))
  with(power2days, lines(Time, as.character(power2days$Sub_metering_3), col = "blue"))
  legend("topright", lty = 1, col = c("black", "red", "blue"), bty = "n", legend = c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"))
  
  plot(power2days$Time, as.character(power2days$Global_reactive_power), type = "l", xlab = "datetime", ylab = "Global_Reactive_Power")
